// <copyright file="SortInventoryChatCommandPlugInBeta.cs" company="MUnique">
...
/// <summary>
/// [DEPRECATED - BETA VERSION] A chat command plugin which sorts the player's inventory by item type, then by item ID, optimizing for item sizes.
/// This is the original implementation that uses MoveItemAction with swap logic.
/// This version may require multiple runs to fully sort the inventory and is disabled by default.
/// Use the main SortInventoryChatCommandPlugIn instead, which uses a more reliable approach.
/// </summary>
[Guid("3cd753e7-010e-4232-8df2-0af8ba4d665c")]
[PlugIn("Sort Inventory (Beta) chat command", "[DEPRECATED] Sorts inventory by item type and ID using MoveItemAction. Usage: /sortinvbeta")]
[ChatCommandHelp(Command, "[DEPRECATED] Sorts inventory by item type, then by item ID, optimizing placement considering item sizes. This beta version may require multiple runs.")]
public class SortInventoryChatCommandPlugInBeta : ChatCommandPlugInBase<EmptyChatCommandArgs>, IDisabledByDefault
{
    private const string Command = "/sortinvbeta";
    ...
    protected override async ValueTask DoHandleCommandAsync(Player player, EmptyChatCommandArgs arguments)
    {
        ...
        // Refresh inventory on client to ensure synchronization
